...
class ContrastMemoryModified(nn.Module):
    ...
    Memory buffer that supplies large amount of negative samples.
    """
    def __init__(self, inputSize, outputSize, K, T=0.07, momentum=0.5, num_clusters=64*20):
    ...
    def forward(self, v1, v2, y, idx=None):
        ...
        kmeans_v1 = faiss.Kmeans(d=inputSize, k=batchSize*20, gpu=True)
        kmeans_v2 = faiss.Kmeans(d=inputSize, k=batchSize*20, gpu=True)
        ...
        idx = torch.arange(batchSize * 20).cuda()
        ...
        weight_v1 = weight_v1.view(batchSize, 20, inputSize)
        out_v2 = torch.bmm(weight_v1, v2.view(batchSize, inputSize, 1))
        out_v2 = torch.exp(torch.div(out_v2, T))

        weight_v2 = torch.index_select(self.centroid_v2, 0, idx).detach()
        weight_v2 = weight_v2.view(batchSize, 20, inputSize)
        out_v1 = torch.bmm(weight_v2, v1.view(batchSize, inputSize, 1))
        out_v1 = torch.exp(torch.div(out_v1, T))

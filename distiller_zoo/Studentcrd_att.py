...
def parse_option():
    ...
    parser.add_argument('--note', type=str, help="Note or description for the training run")
    ...
    parser.add_argument('--model_s', type=str, default='resnet8',
                        choices=['resnet8', 'resnet14', 'resnet20', 'resnet32', 'resnet44', 'resnet56', 'resnet110',
                                 ...
                                 'MobileNetV2', 'ShuffleV1', 'ShuffleV2', 'mobilenet_v2_half'])
    parser.add_argument('--path_t', type=str, default=None, help='teacher model snapshot')

    # distillation
    parser.add_argument('--distill', type=str, default='kd', choices=['kd', 'hint', 'attention', 'similarity',
                                                                      'correlation', 'vid', 'crd', 'kdsvd', 'fsp',
                                                                      'rkd', 'pkt', 'abound', 'factor', 'nst','crd_attention'])
    parser.add_argument('--trial', type=str, default='1', help='trial id')
    parser.add_argument('--sp_weight', type=float, default=0.2, help='weight for similarity-preserving KD')
    parser.add_argument('-r', '--gamma', type=float, default=0.5, help='weight for classification')
    parser.add_argument('-a', '--alpha', type=float, default=None, help='weight balance for KD')
    parser.add_argument('-b', '--beta', type=float, default=None, help='weight balance for other losses')
    parser.add_argument('-d','--delta', type=float, default=0.2, help='Weight for attention loss')
    # KL distillation
...
def main():
    ...
    if opt.dataset == 'cifar100':
        if opt.distill in ['crd', 'attention','crd_attention']:
    ...
    # Define the criterion_list before using it
    criterion_list = nn.ModuleList([])
    criterion_cls = nn.CrossEntropyLoss()
    criterion_div = DistillKL(opt.kd_T)

    # Append classification and divergence loss to criterion_list
    criterion_list.append(criterion_cls)    # classification loss
    criterion_list.append(criterion_div)    # KL divergence loss

    # Add specific distillation loss based on the type of distillation
    if opt.distill == 'kd':
        ...
        criterion_kd = Attention()  # Define your Attention loss class
        criterion_list.append(criterion_kd)
    # Combine CRD and Attention
    elif opt.distill == 'crd_attention':
        opt.s_dim = feat_s[-1].shape[1]
        opt.t_dim = feat_t[-1].shape[1]
        opt.n_data = n_data
        criterion_crd = CRDLoss(opt)
        criterion_attention = Attention()  # Define your Attention loss class
        criterion_kd_hinton = DistillKL(opt.kd_T)
        module_list.extend([criterion_crd.embed_s, criterion_crd.embed_t])
        trainable_list.extend([criterion_crd.embed_s, criterion_crd.embed_t])
        criterion_list.append(criterion_crd)
        criterion_list.append(criterion_attention)
        criterion_list.append(criterion_kd_hinton)
        ...
        module_list.extend([translator, paraphraser])
